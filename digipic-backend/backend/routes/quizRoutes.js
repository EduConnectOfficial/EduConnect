...
router.post('/upload-quiz', uploadRubricMaybe, asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    const { gsUri, publicUrl, metadata } = await saveBufferToStorage(req.file.buffer, {
      destPath, contentType: req.file.mimetype, metadata: { role: 'quiz-rubric', quizId: quizRef.id }
    });
    rubricFile = {
