...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const f of filesArr) {
      // enforce size (multer already caps; this is a friendly safety)
      ...
      const { gsUri, publicUrl, metadata } = await saveBufferToStorage(f.buffer, {
        destPath,
    ...
    return res.status(201).json({ success:true, id: ref.id, assignment: { id: ref.id, ...saved.data() } });
  })
);
...
router.delete('/assignments/:id', asyncHandler(async (req, res) => {
  ...
  await aRef.delete();
  // delete all objects under assignments/{id}/
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const f of files) {
      ...
      fileBlobs.push({
        originalName: f.originalname, size: f.size, mime: f.mimetype,
        gsUri, publicUrl, storageMetadata: metadata
      });
    }
...
async function saveSubmission({ assignmentId, studentId, text, files, bodyClassId = null }) {
  ...
  for (const f of (files || [])) {
    ...
    const destPath = `assignment_submissions/${assignmentId}/${studentId}/${Date.now()}_${safeName(f.originalname || 'file')}`;
    const { gsUri, publicUrl, metadata } = await saveBufferToStorage(f.buffer, {
      destPath, contentType: f.mimetype, metadata: { role: 'assignment-submission', assignmentId, studentId }
    });
    fileBlobs.push({ originalName: f.originalname, size: f.size, mime: f.mimetype, gsUri, publicUrl, storageMetadata: metadata });
  }
